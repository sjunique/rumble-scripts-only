...
public class Player : MonoBehaviour, IBlowable {
	...
	public void AddWind(Vector3 wind){ //Temporary solution. Area should change when wind is comming from different direction
		relativeVelocityAir = transform.InverseTransformVector (flyingBody.linearVelocity - wind);
		flyingBody.AddForce(GliderMath.GetWindForce(wind - flyingBody.linearVelocity, Reference.AREA_PLAYER_FRONT, Reference.DRAG_COEFFICIENT_PLAYER_FRONT));
	}
	...
	private void playerControl(){
		if (!flying) {
			...
			if (Input.GetKeyUp (KeyCode.Space) && flyingBody.linearVelocity.y < 0.5f && flyingBody.linearVelocity.y > -0.5f) { //Deploy the glider. Kind of lags. 
