...
public class Glider : MonoBehaviour, IBlowable{
	...
	private void fly(){ 
		...
		airVelocity = transform.InverseTransformDirection (body.linearVelocity - wind.GetVelocity(this) - thermals.GetVelocity(this));

		//The drag from going forward
		//If the angle of attack is higher, the forward drag will increase
		Vector3 forwardDrag = GliderMath.getDrag (airVelocity.z*Vector3.forward, Reference.DRAG_COEFFICIENT_FRONT, Reference.AIR_DENSITY_20,
		...
		Vector3 fallDrag = GliderMath.getDrag (airVelocity.y * Vector3.up, Reference.DRAG_COEFFICIENT_UNDER, Reference.AIR_DENSITY_20,
			exposedUnder);

		//Drag from side-drifting. Speed is a stabilizer! THIS MAGICALLY CREATED CENTRIFUGAL FORCES!!
		Vector3 sideDrag = GliderMath.getDrag (airVelocity.x * Vector3.right * Mathf.Abs(airVelocity.z), Reference.DRAG_COEFFICIENT_SIDE, Reference.AIR_DENSITY_20,
			                   Reference.AREA_SIDE);
	...
	private Vector3 getLift(float relativeAirSpeed){
		...
		print ("Relative speed: " + relativeAirSpeed + " Actual speed: " + transform.InverseTransformDirection (body.linearVelocity).z + " AoT: " + angleOfAttack + 
	...
	private void brake(){
		...
		Vector3 brakeDrag = GliderMath.getDrag (body.linearVelocity - wind.GetVelocity(this), Reference.DRAG_COEFFICIENT_UNDER, Reference.AIR_DENSITY_20, 
	...
	public void AddWind(Vector3 wind){ //Temporary solution. Area should change when wind is comming from different direction
        body.AddForce (GliderMath.GetWindForce (wind - body.linearVelocity, Reference.AREA_FRONT, Reference.DRAG_COEFFICIENT_FRONT));
}
